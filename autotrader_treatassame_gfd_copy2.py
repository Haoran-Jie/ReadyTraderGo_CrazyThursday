...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            self.new_ask_price = max(bid_prices) + price_adjustment - 2*TICK_SIZE_IN_CENTS if max(bid_prices) != 0 else 0
            self.new_bid_price = min(ask_prices) + price_adjustment + 2*TICK_SIZE_IN_CENTS if min(ask_prices) != 0 else 0
